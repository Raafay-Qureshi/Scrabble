public class Board {
    ...
    private Words words;
    private String availableWords = "  aaaaaaaaabbccddddeeeeeeeeeeeeffggghhiiiiiiiiijkllllmmnnnnnnooooooooppqrrrrrrssssttttttuuuuvvwwxyyz";
    ...
    public char getCharAt(int x, int y) {
        return pieces[x][y].getLetter();
    }

    public void placeWord(String word, int x, int y, char orientation) {
        if (firstWord) {firstWord = false;}

        for (int i = 0; i < word.length(); i++) {
            char letter = word.charAt(i);
            if (orientation == 'h') { 
                pieces[y][x + i].setLetter(letter);
            } else {
                pieces[y + i][x].setLetter(letter);
            }
        }
    }

    public boolean canPlaceWord(String word, int x, int y, char orientation) {
        if (firstWord) {
            for (int i = 0; i < word.length(); i++) {
                if (orientation == 'h') {
                    if (y == 7 && x + i == 7) {
            ...
            for (int i = 0; i < word.length(); i++) {
                ...
                char letter;
                if (orientation == 'h') {
                    letter = pieces[y][x + i].getLetter();
                    sidesValid(y, x + i);
                } else {
                    letter = pieces[y + i][x].getLetter();
                    sidesValid(y + i, x);
                }
    ...
    public boolean sidesValid(int y, int x) {
        ...
        System.out.println(preWord + pieces[y][x].getLetter() + postWord);
        if (postWord != "") {
            if (!words.isWordValid(pieces[y][x].getLetter() + postWord)) {
                return false;
            }
        }
        return true;
    ...
    public void setAvailableWords(String availableWords) {
